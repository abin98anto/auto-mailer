...
import { emailConfig, hotelConfig } from "../config/config";
import { EmailResponse, ReservationDetails } from "../types/Interfaces";
...
export class EmailSender {
  ...
  async sendReservationResponse(
    recipientEmail: string,
    reservationDetails: ReservationDetails,
    originalSubject: string
  ): Promise<boolean> {
    try {
      const response = this.generateReservationResponse(
        reservationDetails,
        originalSubject
      );

      const mailOptions = {
        from: `${hotelConfig.name} <${emailConfig.smtp.user}>`,
        to: recipientEmail,
      ...
      logger.info("Reservation response sent successfully", {
        to: recipientEmail,
      ...
      logger.error("Failed to send reservation response", {
  ...
  private generateTextResponse(details: ReservationDetails): string {
    return `
...
Thank you for choosing ${
      hotelConfig.name
    }. We appreciate your business and look forward to providing you with an exceptional stay.

Best regards,
${hotelConfig.name} Reservations Team

---
This is an automated response. If you need immediate assistance, please call us at ${
      hotelConfig.phone
    }.
    `.trim();
  }

  private generateHtmlResponse(details: ReservationDetails): string {
    return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    ...
    <div class="content">
        ...
        <p>We look forward to welcoming you to ${
          hotelConfig.name
        }. If you have any questions or need to make changes to your reservation, please don't hesitate to contact us.</p>
        
        <div class="hotel-info">
            <h3>Hotel Information</h3>
            <p><strong>${hotelConfig.name}</strong><br>
            ${hotelConfig.address}<br>
            Phone: <a href="tel:${hotelConfig.phone}">${
      hotelConfig.phone
    }</a><br>
            Email: <a href="mailto:${hotelConfig.email}">${
      hotelConfig.email
    }</a></p>
        </div>
        ...
        <div class="footer">
            <p>Thank you for choosing ${
              hotelConfig.name
            }. We appreciate your business and look forward to providing you with an exceptional stay.</p>
            <p><strong>Best regards,</strong><br>
            ${hotelConfig.name} Reservations Team</p>
        </div>
        
        <div class="automated-notice">
            <p>This is an automated response. If you need immediate assistance, please call us at ${
              hotelConfig.phone
            }.</p>
        </div>
    </div>
